...
class TensorFlowSavedModelWrapper(mlflow.pyfunc.PythonModel, ABC):  # type: ignore[reportPrivateImportUsage]
    ...
    def predict(
        ...
        This method is only used during log_model() validation. MLflow calls
        pyfunc predict with a list of input instances, so we iterate and run
        inference per instance. It uses get_input_name_mapping() from the
        subclass to map semantic names to TensorFlow generic names.

        Args:
            context: MLflow context (provided by framework)
            model_input: List of dicts with semantic input names
            params: Optional parameters (unused)

        Returns:
            List of dicts with model outputs as numpy arrays, one per input.
        """
        ...
        for instance in model_input:
            ...
            if isinstance(output, dict):
                results.append({k: v.numpy() for k, v in output.items()})
            else:
