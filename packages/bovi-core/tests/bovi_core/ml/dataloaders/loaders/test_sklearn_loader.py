...
def test_dense_batch_contract(dense_samples):
    loader = SklearnDataLoader(dense_samples, batch_size=2, shuffle=False)
    for _ in range(2):
        batches = list(loader)
...
def test_seeded_epoch_stream_and_metrics_replay(shuffle_samples):
    loaders = [SklearnDataLoader(shuffle_samples, batch_size=7, seed=42) for _ in range(2)]
...
def test_evaluation_iteration_is_stable(shuffle_samples):
    loader = SklearnDataLoader(shuffle_samples, split="validation")
    for _ in range(2):
...
def test_invalid_epoch_rejected(epoch, shuffle_samples):
    loader = SklearnDataLoader(shuffle_samples)
...
class TestSklearnDataLoader:
    ...
    def test_loader_length(self, image_dataset_large):
        ...
        loader = SklearnDataLoader(image_dataset_large, split="train", batch_size=8)
    ...
    def test_loader_iteration(self, image_dataset_large):
    ...
    def test_loader_shuffle(self, image_dataset_large):
        ...
        loader_train = SklearnDataLoader(image_dataset_large, split="train", batch_size=8)
        assert loader_train.shuffle is True

        # Val should not shuffle by default
        loader_val = SklearnDataLoader(image_dataset_large, split="val", batch_size=8)
    ...
    def test_loader_iter_returns_batches(self, image_dataset_large):
        ...
        loader = SklearnDataLoader(image_dataset_large, split="train")
    ...
    def test_loader_get_all_data(self, image_dataset_large):
    ...
    def test_loader_deterministic_shuffle(self, image_dataset_large):
    ...
    def test_loader_empty_dataset(self, tmp_path):
        ...
        loader = SklearnDataLoader(dataset, split="val", batch_size=8)
    ...
    def test_loader_single_sample(self, tmp_path):
        ...
        loader = SklearnDataLoader(dataset, split="train", batch_size=4)

        batches = list(loader)
        assert len(batches) == 1
        assert len(batches[0]["label"]) == 1

    def test_nested_feature_mappings_are_collated(self):
        source = DictSource(
            [
