"""TensorFlow checkpoint failures and epoch-boundary recovery contracts."""
...
def test_incomplete_native_save_retains_metrics(pipeline, tmp_path, monkeypatch, failure_at):
    ...
    calls = 0
    original = type(model.native_model).save

    def fail(native, path):
        nonlocal calls
        calls += 1
        if calls == failure_at:
            Path(path).write_bytes(b"partial")
            raise RuntimeError()
        return original(native, path)

    monkeypatch.setattr(type(model.native_model), "save", fail)
...
def test_output_reuse_does_not_change_old_reference(pipeline, tmp_path):
    ...
    trainer = TensorFlowLinearTrainer(
        model,
        loaders,
        TensorFlowLinearTrainingConfig(epochs=1),
        TrainingContext(run_id=uuid4(), output_dir=tmp_path),
    )
    first = trainer.train()
...
def test_partial_epoch_cancellation_identifies_prior_checkpoint(pipeline, tmp_path, monkeypatch):
    ...
    trainer = TensorFlowLinearTrainer(
        model,
        loaders,
        TensorFlowLinearTrainingConfig(epochs=3),
        TrainingContext(run_id=uuid4(), output_dir=tmp_path),
    )
...
def test_missing_required_bundle_file_is_rejected(pipeline, tmp_path):
    _, definition, loaders = pipeline
    result = TensorFlowLinearTrainer(
        TensorFlowLinearModelProvider().create(definition),
        loaders,
        TensorFlowLinearTrainingConfig(epochs=1),
        TrainingContext(run_id=uuid4(), output_dir=tmp_path),
    ).train()
    ...
    path = resource.local_path.with_suffix(".json")
    path.unlink()
    with pytest.raises(ValueError, match="files"):
        LocalCheckpointResolver().resolve(result.last_checkpoint)


def test_sidecar_write_failure_keeps_prior_bundle(pipeline, tmp_path, monkeypatch):
    _, definition, loaders = pipeline
    provider = TensorFlowLinearModelProvider()
    model = provider.create(definition)
    context = TrainingContext(run_id=uuid4(), output_dir=tmp_path)
    trainer = TensorFlowLinearTrainer(
        model, loaders, TensorFlowLinearTrainingConfig(epochs=1), context
    )
    first = trainer.train()
    assert first.status == "completed", first.issues
    assert first.last_checkpoint is not None
    ...
    failed = trainer.train()
    assert failed.status == "failed"
    assert len(failed.epochs) == 1
    assert failed.last_checkpoint is None
    assert not list((tmp_path / "checkpoints").glob(".pending-*"))
    resource = LocalCheckpointResolver().resolve(first.last_checkpoint)
    assert resource.metadata["run_id"] == str(context.run_id)
    restored = provider.restore_checkpoint(definition, resource)
    np.testing.assert_allclose(restored([[0.5]]), expected, atol=1e-6)
