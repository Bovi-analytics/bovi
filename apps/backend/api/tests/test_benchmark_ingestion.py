...
def test_parse_valid_csv():
    csv_bytes = b"TestId,LactationYield\ncow1,8500.0\ncow2,9200.5\n"
    result = parse_submission_csv(csv_bytes)
    assert result == {"cow1": 8500.0, "cow2": 9200.5}


def test_parse_submission_csv_accepts_legacy_headers():
    csv_bytes = b"cow_id,yield_305day\ncow1,8500.0\n"
    result = parse_submission_csv(csv_bytes)
    assert result == {"cow1": 8500.0}
...
def test_parse_missing_required_column_raises():
    ...
    with pytest.raises(ValueError, match="LactationYield|lactationyield"):
