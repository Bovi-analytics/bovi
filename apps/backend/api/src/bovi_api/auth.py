...
def validate_entra_token(token: str, settings: Settings) -> TokenIdentity:
    ...
    try:
        signing_key = _jwks_client(settings.azure_ad_tenant_id).get_signing_key_from_jwt(token)
        claims = jwt.decode(
            token,
            signing_key.key,
            algorithms=["RS256"],
            audience=[settings.azure_ad_client_id, f"api://{settings.azure_ad_client_id}"],
            issuer=_issuers(settings.azure_ad_tenant_id),
...
async def _ensure_local_user(
    ...
    result = await session.execute(select(User).where(User.entra_oid == identity.entra_oid))
    ...
    if not memberships:
        organization = Organization(name=_default_organization_name(identity))
