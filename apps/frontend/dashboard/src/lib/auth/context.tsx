...
function AuthProvider({ children }: { readonly children: ReactNode }) {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      try {
        ...
        setUser(currentUser);
        setSelectedOrganizationIdState(currentUser.organizations[0]?.id ?? null);
        setAuthMarker(true);
      } catch (error) {
  ...
  const setSelectedOrganizationId = useCallback((organizationId: number) => {
...
function DevAuthProvider({ children }: { readonly children: ReactNode }) {
  const [selectedOrganizationId, setSelectedOrganizationId] = useState<number | null>(1);
